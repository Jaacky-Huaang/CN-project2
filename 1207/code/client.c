...
{       
    ...
    while (1)
    {
        ...
        // Command 1: USER; Command 2: PASS; Command 3: PWD; Command 4: CWD
        // these three commands has similar behavior, so is grouped together
        if (strncmp(input, "USER", 4) == 0 || strncmp(input, "PASS", 4) == 0 || strncmp(input, "PWD", 3) == 0 || strncmp(input, "CWD", 3) == 0)
        {
            ...
            /*
            if (strncmp(input, "PASS", 4) == 0 && login_check(response) == 0)//if successful login
            {
                // we switch to the dedicated folder named after the username
                // send the CWD command to the server automatically
                send(client_socket, "CWD", sizeof("CWD"), 0);
                // receive the response from the server
                memset(response, 0, sizeof(response));
                recv(client_socket, response, sizeof(response), 0);
                // no need to print
